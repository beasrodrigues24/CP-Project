...
outExpAr (Bin op x y) = Right(Right(Left (op, (x,y))))
outExpAr (Un op x) = Right(Right(Right (op, x)))

---
recExpAr g = baseExpAr id id id g g id g
---
g_eval_exp a = either f g
  where f = const a
        g = either h j 
        h (b) = b
        j = either k l 
...
\start
   | out . in = id |
%
\just\equiv{ in definition, fusion-+, universal property ,  }
%
\left\{
   \begin{array}{lll}
       |out . X = i1|\\
...
\right.
%
\just\equiv{ fusion-+ ,  universal property, ops definition }
%
\left\{
   \begin{array}{lll}
       |out . X = i1|\\
...
\right.
%
\just\equiv{ fusion-+ , universal property }
%
\left\{
   \begin{array}{lll}
       |out . X = i1|\\
...
\right.
%
\just\equiv{ extensional equality , def-comp }
%
\left\{
   \begin{array}{lll}
       |out X = i1()|\\
       |out (N a) = i2 (i1 a)|\\
       |out (bin (op, (x,y))) = i2 (i2 (i1 (op, (x,y))))|\\
       |out (Uncurry Un (op, x)) = i2 (i2 (i2 (op, x)))|
   \end{array}
\right.
%
\just\equiv{ def op , def uncurry }
%
\left\{
   \begin{array}{lll}
       |out X = i1()|\\
...
&
    |1 + (A + (B X (ExpAr a X ExpAr a) + C X ExpAr a))|
...
&
    |1 + (A + (B X (Expr a X (Expr a) + C X Expr a))|
           \ar[d]^{|recExpAr (cataExpAr g_eval_exp)|}
           \ar[l]_-{|inExpAr|}
\\
     |Nat0|
&
     |1 + (A + (B X (Nat0, Nat0) + C X Nat0))|
           \ar[l]^-{|g_eval_exp|}
}
\end{eqnarray*}\\

Para a alínea 3 , criou-se um anamorfismo que limpa os casos especiais da soma, do produto, da negação e da exponenciação. 
O catamorfismo é muito semelhante ao \emph{gEvalExp}, mas também tem em conta os ditos casos especiais.
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |ExpAr a|
...
\\
     |ExpAr a|
           \ar[d]_-{|cataExpAr gopt|}
&
     |1 + (A + (B X (ExpAr a X ExpAr a) + C X ExpAr a))|
           \ar[d]^{|recExpAr (cataExpAr gopt)|}
           \ar[l]^-{|inExpAr|}
\\ 
     |Nat0|
& 
     |1 + (A + (B X (Nat0 X Nat0) + C X Nat0))|
           \ar[l]^-{|gopt|}
}
\end{eqnarray*}\\

Relativamente à alínea 4, para o cálculo da derivada era essencial mantermos acessível o valor da expressão sem derivaçao para além do valor derivado porque, por exemplo, derivar um produto exige conhecê-la. 
Assim o gene do catamorfismo devolve sempre um par no qual o primeiro elemento é a expressão e o segundo elemento é a mesma expressão mas derivada de acordo com as regras comuns da derivação.\\
...
\xymatrix@@C=2cm{
    |ExpAr a|
           \ar[d]_-{|cataExpAr sd_gen|}
&
    |1 + (A + (B X (ExpAr a X ExpAr a) + C X ExpAr a))|
           \ar[d]^{|recExpAr(cataExpAr sd_gen)|}
           \ar[l]_-{|inExpAr|}
\\
     |ExpAr a X ExpAr a|
&
...
expr =  1 + (A + (B X ((ExpAr a,ExpAr a) X (ExpAr a, ExpAr a)) + C X (ExpAr a,ExpAr a))) \\
...
\xymatrix@@C=2cm{
    |ExpAr a|
           \ar[d]_-{|cataExpAr ad_gen|}
&
    |1 + (A + (B X (ExpAr a X ExpAr a) + C X ExpAr a))|
           \ar[d]^{|recExpAr(cataExpAr ad_gen)|}
           \ar[l]_-{|inExpAr|}
\\
     |Nat0 X Nat0|
&
     |1 + (A + (B X ((Nat0, Nat0) X (Nat0, Nat0)) + C X (Nat0, Nat0)))|
           \ar[l]^-{|ad_gen|}
}
\end{eqnarray*}
...
loop (tot, up, down, up_inc, down_inc) 
  = (div up down, up * (up_inc + 1) * up_inc, down * (down_inc + 1) * down_inc, up_inc + 2, down_inc + 1)
inic = (1, 1, 1, 3, 2)
prj (a, b, c, d, e) = a
\end{code}
por forma a que
...
Para resolver o problema 2, o problema foi abordado da seguinte forma: 
tendo (2n)! no numero e n! e (n+1)! no denominador, sempre que realizar um produto em baixo, 2 produtos terão que ser feitos em cima.\\
Por exemplo:\\

((2n x (2n-1))  X  (((2n-2) x (2n-3))  X  ...  X   4 x 3\\

dividirá por\\

  ((n+1) x n))   X    (n x (n-1)))      X  ... X   3 x 2\\

Assim foi criada um função up , responsável pelos produtos no numerador e a função down responsável pelos produtos no denominador. 
\emph{DownInc} e \emph{upInc} permitem ter disponíveis os valores a multiplicar no momento.
...
deCasteljau = hyloAlgForm alg coalg where
   ...
   f [] = nil
   f p = const p 
   g ((op1,op2)) = \pt -> (calcLine (op1 pt) (op2 pt)) pt 

hyloAlgForm = hyloLTree
\end{code}

Sendo o \emph{calcLine} o catamorfismo de um dado gene h, o essencial era definir este gene. 
Para isso, foi necessário basear a solução no seguinte diagrama.
...
&
    |1 + Q X NPoint|
           \ar[d]^{|recList (cataList h)|}
\\
     |NPoint -> Overtime NPoint|
&
     |1 + Q X (NPoint -> Overtime NPoint)|
           \ar[l]^-{|h|}
}
\end{eqnarray*}

Desta forma, o gene h dividia-se no caso nulo e no caso Q X (NPoint -> Overtime NPoint).

ambos definidos na função disponibilizada no enunciado e sendo só necessário adaptar.\\

Para a definição do \emph{deCasteljau}, identificou-se que a estrutura intermediária apropriada para o hilomorfismo seria uma Leaf tree. 
Usando esta estrutura, era possível partir a lista num ponto e em duas listas de pontos, que depois do anamorfismo tornam a execução do catamorfismo mais eficiente , devido ao tipo NPoint + LTree NPoint x LTree NPoint . 
O catamorfismo segue o mesmo algoritmo aplicado na função disponibilizada.

\begin{eqnarray*}
\xymatrix@@C=2cm{
    |[NPoint]|
           \ar[d]_-{|anaLTree coalg|}
           \ar[r]_-{|coalg|}
&
    |NPoint + [NPoint] X [NPoint]|
           \ar[d]^-{|recLTree (anaLTree coalg)|}
\\
    |LTree NPoint|
           \ar[d]_-{|cataLTree alg|}
           \ar[r]_-{|out|}
&
    |NPoint + LTree NPoint X LTree NPoint|
           \ar[d]^-{|recLTree (cataLTree alg)|}
\\
    |Overtime NPoint|      
&
    |NPoint + Overtime NPoint X Overtime NPoint|
          \ar[l]_-{|alg|}
}
\end{eqnarray*}
...
\begin{code}
inNEmpList = either singl cons 

outNEmpList [a] = i1(a)
outNEmpList (a:x) = i2(a, x)

recNEmpList f = id -|- id >< f

cataNEmpList g = g . recNEmpList(cataNEmpList g) . outNEmpList 

avg_aux = cataNEmpList(gene)
  where gene = either b q 
        b = split id (const 1)
        q  = split avg length
        avg (a, (x,y)) = (a + x*y)/(y+1) 
        length (_, (_, y)) = y + 1
...
Para o \emph{avgAaux} relembro que o tipo da List é A + A x List a. Então no gene, primeiro separamos nos 2 casos com um either b q.\\ 

  - o b representa o tipo A isolado, o nosso caso de paragem. 
    Quando chegarmos a este caso vamos querer adicionar o valor do elemento da lista e sinalizar +1 para o comprimento da lista, guardando isso num par através de um split.\\
  - o q representa o tipo A x List a. Então dividimos o q em 2 tarefas: calcular a média num lado, atualizar o comprimento do outro. \\

O avg e o length recebem argumentos do tipo (a, (b,c)) em que o 'a' é o valor do nodo em que estamos atualmente e o par (b,c) é o que já se calculou recursivamente. 
Então pela fórmula, a média dá-se daquela fórmula e incrementa-se um ao comprimento da lista.
...
\begin{code} data LTree a = Leaf a | Fork (LTree a, LTree a) \end{code}.
Temos agora algo do tipo A + LTree x LTree . Então o nosso catamorfismo também vai gerar um par com a média de um lado e o número de nodos com valor do outro mas, agora nós so temos valores nas folhas.\\
Desta forma, dividimos em 2 partes com um either f g. \\
  ...
  - o g terá o tipo LTree x LTree e nesses casos vamos só querer ter atenção ao que foi encontrado recursivamente nas folhas e juntar. \\
Então g dá split em avg e length também para gerar o par e ambos têm tipo ((a,b), (c,d)) em que (a,b) é o par (avg,length) da LTree a da esquerda e o par (c,d) é o par da direita, obtidos recursivamente. \\
